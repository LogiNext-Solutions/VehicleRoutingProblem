...
public class  VRP{
    ...
    public static void main(String[] args) {
        ...
        NoOfVehicles=sc.nextInt();
        System.out.println("Enter capacity of vehicles");
        VehicleCap=sc.nextInt();
        ...
        for (int i = 0; i <= NoOfCustomers; i++) {
            for (int j = i + 1; j <= NoOfCustomers; j++) //The table is symmetric to the first diagonal
        ...
        int printMatrix = 0; //If we want to print distance matrix
            ...
            if ( Candidate  == null)
            {
                ...
                else //We DO NOT have any more vehicle to assign. The problem is unsolved under these parameters
                {
                    System.out.println("\nThe rest customers do not fit in any Vehicle\n" +
                            "The problem cannot be resolved under these constraints");
                    System.exit(0);
                }
            }
            else
